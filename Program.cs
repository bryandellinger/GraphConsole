...
while (choice != 0)
{
    ...
    Console.WriteLine("5. List Edu events using delegation permission");
    Console.WriteLine("6. List Edu events using application permission");
    ...
    switch(choice)
    {
        ...
        case 5:
            // Run any Graph code
            await MakeGraphCallAsyncDelegate();
            break;
        case 6:
            // Run any Graph code
            await MakeGraphCallAsyncApplication();
            break;
        default:
...
async Task MakeGraphCallAsyncDelegate()
{
    try
    {
        var events = await GraphHelper.GetEventsDelegate();

        // Output each users's details
    
         //   Console.WriteLine($"Name: {group.DisplayName ?? "NO NAME"}");
         //   Console.WriteLine($"  ID: {group.Id}");
         //   Console.WriteLine($" Description: {group.Description ?? "NO EMAIL"}");

        foreach (var evt in events)
        {
            Console.WriteLine($"event: {evt.Subject ?? "NO Subject"}");
        }
...
// </MakeGraphCallSnippet>
async Task MakeGraphCallAsyncApplication()
{
    try
    {
        var events = await GraphHelper.GetEventsApplication();
